...
function projectTheme_workspace_chats_scr() {
	...
	<div id="usual2" class="usual">
       ...
       There are no chats yet in the website.
...
function projectTheme_reviews_scr() {
    ...
    if(isset($_POST['approverate'])){
        if(isset($_POST['user_id']) && isset($_POST['project_id'])){
            $user_id = $_POST['user_id'];
            $project_id = $_POST['project_id'];
            $querystr = "update ".$wpdb->prefix."project_ratings set approved = 1 WHERE awarded = 1 AND touser='$user_id' AND pid='$project_id'";
            $wpdb->query($querystr); 
        }
    ...
    <div id="usual2" class="usual">
        ...
        <?php
            $s = "select * from ".$wpdb->prefix."project_ratings where awarded = 1 order by id desc";
            $r = $wpdb->get_results($s);
        ...
        <table class="widefat post fixed" cellspacing="0">
            <thead>
                <tr>
                    ...
                    <th><?php _e('Consistency Grade','ProjectTheme'); ?></th>
                    <th><?php _e('Communication Grade','ProjectTheme'); ?></th>
                    <th><?php _e('Review','ProjectTheme'); ?></th>
                    <th><?php _e('Personal Note','ProjectTheme'); ?></th>
                    <th><?php _e('Want to work again','ProjectTheme'); ?></th>
                    <th><?php _e('Payment Method','ProjectTheme'); ?></th>
            ...
            <tbody>
                <?php
                    foreach($r as $row) {
                        ...
                        echo '<tr> 
                            <form method="post" action="'.home_url().'/greysecretsell/admin.php?page=user-reviews&admin_bar=1">';
                                ...
                                echo '<th>'.$row->consistency_grade.'</th>';
                                echo '<th>'.$row->communication_grade.'</th>';
                                echo '<th>'.$row->review_comments.'</th>';
                                echo '<th>'.$row->personal_review.'</th>';
                                echo '<th>'.$row->work_again.'</th>';
                                echo '<th>'.$row->pay_method.'</th>';
                                echo '<th>'.date('d-M-Y H:i:s', $row->datemade).'</th>';
                                echo '<th>';
                                    if($row->approved == 0):
                                        echo '<input type="submit" name="approverate" value="Approve rate">';
...
function projectTheme_payment_methods() {
	...
	if(!function_exists('PGG_wan_load_textdomain'))
	{ ?>

	<div class='updated' style="background:#FEFBD7; color: #000 !important"><p>
			You do not have extra dev license, which means you are missing out a few other payment gateways like: <b>stripe, paystack, payumoney, mangopay, voguepay, pagoseguro, iDeal, braintree, authorize.net, epay.dk, pesapal, <br/>
			robokasa, simplepay.ng, quickpay, bitcoins, payfast, payflow and others</b>. You can login to your account in <a href='https://sitemile.com' target='_blank'>www.sitemile.com</a> and upgrade to extra dev license for your theme. </p>
	</div>
          ...
          <form method="post" action="<?php echo get_admin_url(); ?>admin.php?page=payment-methods&active_tab=tabs1">
            <table width="100%" class="sitemile-table">
                    ...
                    <td>
                     ...
                     - <a target="_blank"  href="https://developer.paypal.com/docs/classic/lifecycle/goingLive/">Going Live with PayPal APP</a> <br/>
               ...
               Our theme makes use of the delayed chained payments. The payments are in the Implicit Approval Mode. This only applies when you set adaptive payments to <strong>YES</strong>.
...
function projectTheme_email_settings() {
    ...
    if(isset($_POST['ProjectTheme_retract_bid_bidder_email_save'])) {
		update_option('ProjectTheme_retract_bid_bidder_email_message', 	trim($_POST['ProjectTheme_retract_bid_bidder_email_message']));
		update_option('ProjectTheme_retract_bid_bidder_email_subject', 	trim($_POST['ProjectTheme_retract_bid_bidder_email_subject']));

		echo '<div class="saved_thing">'.__('Settings saved!','ProjectTheme').'</div>';
    }

    if(isset($_POST['ProjectTheme_retract_bid_owner_email_save'])) {
		update_option('ProjectTheme_retract_bid_owner_email_message', 	trim($_POST['ProjectTheme_retract_bid_owner_email_message']));
		update_option('ProjectTheme_retract_bid_owner_email_subject', 	trim($_POST['ProjectTheme_retract_bid_owner_email_subject']));

		echo '<div class="saved_thing">'.__('Settings saved!','ProjectTheme').'</div>';
    }

    if(isset($_POST['ProjectTheme_your_new_password_email_save'])) {
		update_option('ProjectTheme_your_new_password_email_subject', 	trim($_POST['ProjectTheme_your_new_password_email_subject']));
		update_option('ProjectTheme_your_new_password_email_meesage', 	trim($_POST['ProjectTheme_your_new_password_email_meesage']));

		echo '<div class="saved_thing">'.__('Settings saved!','ProjectTheme').'</div>';
    }

    if(isset($_POST['ProjectTheme_new_user_how_it_works_email_save'])) {
		update_option('ProjectTheme_new_user_how_it_works_email_subject', 	trim($_POST['ProjectTheme_new_user_how_it_works_email_subject']));
		update_option('ProjectTheme_new_user_how_it_works_email_message', 	trim($_POST['ProjectTheme_new_user_how_it_works_email_message']));
    ...
    if(isset($_POST['ProjectTheme_reset_password_submit'])) {
		update_option('ProjectTheme_reset_password_email_subject', 	trim($_POST['ProjectTheme_reset_password_email_subject']));
		update_option('ProjectTheme_reset_password_email_message', 	trim($_POST['ProjectTheme_reset_password_email_message']));
    ...
    if(isset($_POST['ProjectTheme_approve_rate_email_save'])) {
        update_option('ProjectTheme_approve_rate_email_enable',     trim($_POST['ProjectTheme_approve_rate_email_enable']));
        update_option('ProjectTheme_approve_rate_email_subject', 	trim($_POST['ProjectTheme_approve_rate_email_subject']));
        update_option('ProjectTheme_approve_rate_email_message', 	trim($_POST['ProjectTheme_approve_rate_email_message']));
	...
	<div id="usual2" class="usual">
        <ul>
            ...
            <li><a href="#reset_password"><?php _e('Reset Your Password','ProjectTheme'); ?></a></li>
            <li><a href="#your_new_password"><?php _e('Your New Password','ProjectTheme'); ?></a></li>
            <li><a href="#new_user_how_it_works_email"><?php _e('New User How It Works Email','ProjectTheme'); ?></a></li>
            ...
            <li><a href="#approve_rate"><?php _e('Approve Rate (Admin)','ProjectTheme'); ?></a></li>
            ...
            <li><a href="#retract_bid_owner"><?php _e('Retract Bid (Owner)','ProjectTheme'); ?></a></li>
            <li><a href="#retract_bid_bidder"><?php _e('Retract Bid (bidder)','ProjectTheme'); ?></a></li>
        ...
        <div id="escrow_project_bidder">
            <div class="spntxt_bo">
                <?php _e('This email will be received by the provider/bidder when the project owner puts the money into escrow.
            ...
            <form method="post" action="<?php echo get_admin_url(); ?>admin.php?page=email-settings&active_tab=escrow_project_bidder">
                <table width="100%" class="sitemile-table">
                    ...
                    <tr>
                        ...
                        <td ></td>
                        <td>
                            <input type="submit"  class="button button-primary button-large" name="ProjectTheme_escrow_project_bidder_email_save"
                            value="<?php _e('Save Options','ProjectTheme'); ?>"/>
                        </td>
                    </tr>
        ...
        <div id="retract_bid_bidder">

            <div class="spntxt_bo">
                <?php _e('This email will be received by the admin when a user creates a rating. ','ProjectTheme'); ?>
            </div>

            <form method="post" action="<?php echo get_admin_url(); ?>admin.php?page=email-settings&active_tab=retract_bid_bidder">
                <table width="100%" class="sitemile-table">

                    <tr>
                        <td valign=top width="22"><?php ProjectTheme_theme_bullet(); ?></td>
                        <td ><?php _e('Enable this email:','ProjectTheme'); ?></td>
                        <td><?php echo ProjectTheme_get_option_drop_down($arr, 'ProjectTheme_retract_bid_bidder_email_enable'); ?></td>
                    </tr>

                    <tr>
                        <td valign=top width="22"><?php ProjectTheme_theme_bullet(); ?></td>
                        <td width="160"><?php _e('Email Subject:','ProjectTheme'); ?></td>
                        <td><input type="text" size="90" name="ProjectTheme_retract_bid_bidder_email_subject" value="<?php echo stripslashes(get_option('ProjectTheme_retract_bid_bidder_email_subject')); ?>"/></td>
                    </tr>

                    <tr>
                        <td valign=top><?php ProjectTheme_theme_bullet(); ?></td>
                        <td valign=top ><?php _e('Email Content:','ProjectTheme'); ?></td>
                        <td><textarea cols="92" rows="10" name="ProjectTheme_retract_bid_bidder_email_message"><?php echo stripslashes(get_option('ProjectTheme_retract_bid_bidder_email_message')); ?></textarea></td>
                    </tr>

                    <tr>
                        <td ></td>
                        <td ></td>
                        <td><input type="submit"  class="button button-primary button-large" name="ProjectTheme_retract_bid_bidder_email_save" value="<?php _e('Save Options','ProjectTheme'); ?>"/></td>
                    </tr>
        ...
        <div id="retract_bid_owner">

            <div class="spntxt_bo">
                <?php _e('This email will be received by the admin when a user creates a rating. ','ProjectTheme'); ?>
            </div>

            <form method="post" action="<?php echo get_admin_url(); ?>admin.php?page=email-settings&active_tab=retract_bid_owner">
                <table width="100%" class="sitemile-table">

                    <tr>
                        <td valign=top width="22"><?php ProjectTheme_theme_bullet(); ?></td>
                        <td ><?php _e('Enable this email:','ProjectTheme'); ?></td>
                        <td><?php echo ProjectTheme_get_option_drop_down($arr, 'ProjectTheme_retract_bid_owner_email_enable'); ?></td>
                    </tr>

                    <tr>
                        <td valign=top width="22"><?php ProjectTheme_theme_bullet(); ?></td>
                        <td width="160"><?php _e('Email Subject:','ProjectTheme'); ?></td>
                        <td><input type="text" size="90" name="ProjectTheme_retract_bid_owner_email_subject" value="<?php echo stripslashes(get_option('ProjectTheme_retract_bid_owner_email_subject')); ?>"/></td>
                    </tr>

                    <tr>
                        <td valign=top><?php ProjectTheme_theme_bullet(); ?></td>
                        <td valign=top ><?php _e('Email Content:','ProjectTheme'); ?></td>
                        <td><textarea cols="92" rows="10" name="ProjectTheme_retract_bid_owner_email_message"><?php echo stripslashes(get_option('ProjectTheme_retract_bid_owner_email_message')); ?></textarea></td>
                    </tr>

                    <tr>
                        <td ></td>
                        <td ></td>
                        <td><input type="submit"  class="button button-primary button-large" name="ProjectTheme_retract_bid_owner_email_save" value="<?php _e('Save Options','ProjectTheme'); ?>"/></td>
                    </tr>
        ...
        <div id="your_new_password">

            <div class="spntxt_bo">
                <?php _e('This email will be received by the admin when a user creates a rating. ','ProjectTheme'); ?>
            </div>

            <form method="post" action="<?php echo get_admin_url(); ?>admin.php?page=email-settings&active_tab=your_new_password">
                <table width="100%" class="sitemile-table">

                    <tr>
                        <td valign=top width="22"><?php ProjectTheme_theme_bullet(); ?></td>
                        <td ><?php _e('Enable this email:','ProjectTheme'); ?></td>
                        <td><?php echo ProjectTheme_get_option_drop_down($arr, 'ProjectTheme_your_new_password_email_enable'); ?></td>
                    </tr>

                    <tr>
                        <td valign=top width="22"><?php ProjectTheme_theme_bullet(); ?></td>
                        <td width="160"><?php _e('Email Subject:','ProjectTheme'); ?></td>
                        <td><input type="text" size="90" name="ProjectTheme_your_new_password_email_subject" value="<?php echo stripslashes(get_option('ProjectTheme_your_new_password_email_subject')); ?>"/></td>
                    </tr>

                    <tr>
                        <td valign=top><?php ProjectTheme_theme_bullet(); ?></td>
                        <td valign=top ><?php _e('Email Content:','ProjectTheme'); ?></td>
                        <td><textarea cols="92" rows="10" name="ProjectTheme_your_new_password_email_meesage"><?php echo stripslashes(get_option('ProjectTheme_your_new_password_email_meesage')); ?></textarea></td>
                    </tr>

                    <tr>
                        <td ></td>
                        <td ></td>
                        <td><input type="submit"  class="button button-primary button-large" name="ProjectTheme_your_new_password_email_save" value="<?php _e('Save Options','ProjectTheme'); ?>"/></td>
                    </tr>
        ...
        <div id="new_user_how_it_works_email">

            <div class="spntxt_bo">
                <?php _e('This email will be received by a new user in order for him to see how GreySell works. ','ProjectTheme'); ?>
            </div>

            <form method="post" action="<?php echo get_admin_url(); ?>admin.php?page=email-settings&active_tab=new_user_how_it_works_email">
                <table width="100%" class="sitemile-table">

                    <tr>
                        <td valign=top width="22"><?php ProjectTheme_theme_bullet(); ?></td>
                        <td ><?php _e('Enable this email:','ProjectTheme'); ?></td>
                        <td><?php echo ProjectTheme_get_option_drop_down($arr, 'ProjectTheme_new_user_how_it_works_email_enable'); ?></td>
                    </tr>

                    <tr>
                        <td valign=top width="22"><?php ProjectTheme_theme_bullet(); ?></td>
                        <td width="160"><?php _e('Email Subject:','ProjectTheme'); ?></td>
                        <td><input type="text" size="90" name="ProjectTheme_new_user_how_it_works_email_subject" value="<?php echo stripslashes(get_option('ProjectTheme_new_user_how_it_works_email_subject')); ?>"/></td>
                    </tr>

                    <tr>
                        <td valign=top><?php ProjectTheme_theme_bullet(); ?></td>
                        <td valign=top ><?php _e('Email Content:','ProjectTheme'); ?></td>
                        <td><textarea cols="92" rows="10" name="ProjectTheme_new_user_how_it_works_email_message"><?php echo stripslashes(get_option('ProjectTheme_new_user_how_it_works_email_message')); ?></textarea></td>
                    </tr>

                    <tr>
                        <td ></td>
                        <td ></td>
                        <td><input type="submit"  class="button button-primary button-large" name="ProjectTheme_new_user_how_it_works_email_save" value="<?php _e('Save Options','ProjectTheme'); ?>"/></td>
                    </tr>
        ...
        <div id="approve_rate">
            ...
            <form method="post" action="<?php echo get_admin_url(); ?>admin.php?page=email-settings&active_tab=approve_rate">
                <table width="100%" class="sitemile-table">

                    <tr>
                        <td valign=top width="22"><?php ProjectTheme_theme_bullet(); ?></td>
                        <td ><?php _e('Enable this email:','ProjectTheme'); ?></td>
                        <td><?php echo ProjectTheme_get_option_drop_down($arr, 'ProjectTheme_approve_rate_email_enable'); ?></td>
                    </tr>

                    <tr>
                        <td valign=top width="22"><?php ProjectTheme_theme_bullet(); ?></td>
                        <td width="160"><?php _e('Email Subject:','ProjectTheme'); ?></td>
                        <td><input type="text" size="90" name="ProjectTheme_approve_rate_email_subject" value="<?php echo stripslashes(get_option('ProjectTheme_approve_rate_email_subject')); ?>"/></td>
                    </tr>

                    <tr>
                        <td valign=top><?php ProjectTheme_theme_bullet(); ?></td>
                        <td valign=top ><?php _e('Email Content:','ProjectTheme'); ?></td>
                        <td><textarea cols="92" rows="10" name="ProjectTheme_approve_rate_email_message"><?php echo stripslashes(get_option('ProjectTheme_approve_rate_email_message')); ?></textarea></td>
                    </tr>

                    <tr>
                        <td ></td>
                        <td ></td>
                        <td><input type="submit"  class="button button-primary button-large" name="ProjectTheme_approve_rate_email_save" value="<?php _e('Save Options','ProjectTheme'); ?>"/></td>
                    </tr>
        ...
        <div id="bid_project_subs">
          ...
          <div id="payment_on_completed_project">
            <div class="spntxt_bo">
                <?php _e('This email will be received by the project owner when he pays the service provider.
                Be aware, if you add html tags to this email you must have the allow HTML tags option set to yes.
                Also at the bottom you can see a list of tags you can use in the email body.','ProjectTheme'); ?>
            </div>
            ...
            <form method="post" action="<?php echo get_admin_url(); ?>admin.php?page=email-settings&active_tab=payment_on_completed_project">
                <table width="100%" class="sitemile-table">
                    ...
                    <tr>
                        ...
                        <td ></td>
                        <td>
                            <input type="submit"  class="button button-primary button-large" name="ProjectTheme_payment_on_completed_project_email_save" value="<?php _e('Save Options','ProjectTheme'); ?>"/>
                        </td>
                    </tr>
        ...
        <div id="new_user_email" style="display: none; ">
        	<div class="spntxt_bo">
                <?php _e('This email will be received by all new users who register on your website.
                Be aware, if you add html tags to this email you must have the allow HTML tags option set to yes.
                Also at the bottom you can see a list of tags you can use in the email body.','ProjectTheme'); ?> </div>
        ...
        <div id="reset_password" style="display: none; ">
        	<div class="spntxt_bo">
                <?php _e('This email will be received by all users when they want to reset their password.','ProjectTheme'); ?> </div>

                <form method="post" action="<?php echo get_admin_url(); ?>admin.php?page=email-settings&active_tab=reset_password">
                    <table width="100%" class="sitemile-table">
                        <tr>
                            <td valign=top width="22"><?php ProjectTheme_theme_bullet(); ?></td>
                            <td width="160"><?php _e('Email Subject:','ProjectTheme'); ?></td>
                            <td><input type="text" size="90" name="ProjectTheme_reset_password_email_subject" value="<?php echo stripslashes(get_option('ProjectTheme_reset_password_email_subject')); ?>"/></td>
                        </tr>

                        <tr>
                            <td valign=top><?php ProjectTheme_theme_bullet(); ?></td>
                            <td valign=top ><?php _e('Email Content:','ProjectTheme'); ?></td>
                            <td><textarea cols="92" rows="10" name="ProjectTheme_reset_password_email_message"><?php echo stripslashes(get_option('ProjectTheme_reset_password_email_message')); ?></textarea></td>
                        </tr>
                        ...
                        <tr>
                            <td ></td>
                            <td ></td>
                            <td><input type="submit"  class="button button-primary button-large" name="ProjectTheme_reset_password_submit" value="<?php _e('Save Options','ProjectTheme'); ?>"/></td>
                        </tr>
                </table>
            </form>
        </div>
...
function projectTheme_pricing_options() {
	...
	global $menu_admin_projecttheme_theme_bull, $wpdb;
		...
		$arr_currency = array("USD" => "US Dollars", "EUR" => "Euros", "CAD" => "Canadian Dollars", "CHF" => "Swiss Francs","GBP" => "British Pounds",
						  ...
						  'SEK' => 'Swedish Krona', 'Israeli New Shekel' => 'ILS', 'COP' => 'Colombian Peso', 'THB' => 'Thai Baht'
						  );
