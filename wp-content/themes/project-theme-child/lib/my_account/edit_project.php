...
if(!is_user_logged_in()) { wp_redirect(home_url()."/wp-login.php"); exit; }
	...
	//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
		...
		if(isset($_POST['save-project']))
		{
			...
			$project_end 					= trim($_POST['project_ending']);

			if(empty($project_end)) $project_ending = current_time('timestamp',0) + 30*3600*24;
			else $project_ending = strtotime($project_end, current_time('timestamp',0));

			if(!empty($projectTheme_project_period))
			{
				$project_ending1 = current_time('timestamp',0) + $projectTheme_project_period*3600*24;
				if($project_ending > $project_ending1) $project_ending = $project_ending1;
				if($project_ending < current_time('timestamp',0)) $project_ending = $project_ending1;
			}

			if($project_ending < current_time('timestamp',0)) $project_ending = current_time('timestamp',0) + 30*3600*24;


			$finalised_posted = get_post_meta(get_the_ID(),'finalised_posted',true);
			if($finalised_posted != "1") {
				update_post_meta($pid, "project_ending", 		$project_ending); 	// ending date for the project
				update_post_meta($pid, "ending", 		$ending); 					// ending date for the project bids
			}
...
<div class="account-main-area col-xs-12 col-sm-8 col-md-8 col-lg-8">
    ...
    <ul class="post-new3">
        	...
        	<p><input type="text" size="50" class="do_input_new full_wdth_me" name="project_title"
                  ...
                  <li>
          			...
          			<?php
          			 	...
          			 	echo projectTheme_get_categories_clck("project_cat",
                                          !isset($_POST['project_cat_cat']) ? (is_array($cat) ? $cat[1]->term_id : "") : htmlspecialchars($_POST['project_cat_cat'])
	   ...
	   $project_dt = get_post_meta($pid,'project_ending',true);

	   if(!empty($project_dt))
	   $project_dt = date_i18n('d-m-Y',$project_dt);

	   ?>

       <?php _e("Project Ending On",'ProjectTheme'); ?>:</h2>
       <p><input type="text" name="ending" id="ending" class="do_input_new full_wdth_me" value="<?php echo $dt; ?>"  /></p>
	   </li>
	   ...
	   <script>
		var $ = jQuery;
			$(document).ready(function () {
				...
				$('#project_ending').datepicker({
					dateFormat: "dd-M-yy",
					onClose: function () {
						var dt1 = $('#ending').datepicker('getDate');
						var dt2 = $('#project_ending').datepicker('getDate');
						if (dt2 <= dt1) {
							var minDate = $('#project_ending').datepicker('option', 'minDate');
							$('#project_ending').datepicker('setDate', minDate);
						}
					}
				});
			});
	   ...
	   <li> <h2>
	   <?php _e("The Project Ending On",'ProjectTheme'); ?>:</h2>
       <p><input type="text" name="project_ending" id="project_ending" class="do_input_new full_wdth_me" value="<?php echo $project_dt; ?>"  /></p>
       </li>
	  ...
	  ?>
        ...
        <script>
			...
			$('#location_select').on('change', function() {
				if(this.value == "266") {
					$('#change_address_field').hide();
				} else {
					$('#change_address_field').show();
				}
			});
		</script>

        <li id = "change_address_field">
