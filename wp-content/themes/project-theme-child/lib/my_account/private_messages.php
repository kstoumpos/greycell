    ...
    <div class="account-main-area col-xs-12 col-sm-8 col-md-8 col-lg-8"> 
        <?php
			if($third_page == 'home') {
				...
				<div class="my_box3">
					<div class="padd10">
						<div class="clear10"></div>
						<ul class="cms_cms_messages">
							<!-- <li><a href="<?php //echo $home_url . "/ο-λογαριασμός-μου/προσωπικά-μηνύματα/?rdr=&pg=home"; ?>" class="green_btn"><?php //_e("Messaging Home","ProjectTheme"); ?></a></li> -->
							<li> <a href="<?php echo $home_url . "/ο-λογαριασμός-μου/προσωπικά-μηνύματα/?rdr&pg=send"; ?>" class="green_btn"><?php _e("Send New Message","ProjectTheme"); ?></a></li>
							<li><a href="<?php echo $home_url . "/ο-λογαριασμός-μου/προσωπικά-μηνύματα/?rdr=&pg=inbox"; ?>" class="green_btn"><?php _e("Inbox","ProjectTheme");

							$rd = projectTheme_get_unread_number_messages($current_user->ID);
							if($rd > 0) echo ' ('.$rd.')';

							?></a></li>
							<li><a href="<?php echo $home_url . "/ο-λογαριασμός-μου/προσωπικά-μηνύματα/?rdr=&pg=sent-items"; ?>" class="green_btn"><?php _e("Sent Items","ProjectTheme"); ?></a></li>
            	...
            	<div class="my_box3">
            		<div class="box_title">
						<?php _e("Latest Received Messages","ProjectTheme"); ?>
					</div>
							...
							if(count($r) > 0) {
								echo '<table width="100%" class="da_table_done">';
									...
									foreach($r as $row) {
										...
										echo '<tr '.$bold.'>';
										echo '<td class="'.$cls.'">
												<img src="'.ProjectTheme_get_avatar($user_nv->ID,25, 25). '" alt="avatar-user" class="acc_m1" width="25" height="25" />
												<a href="'.home_url().'/?p_action=user_profile&post_author='.$user_nv->ID.'">'.$user_nv->user_login.'</a>
											</td>';
            	...
            	<div class="my_box3">
            		<div class="box_title">
						<?php _e("Latest Sent Items","ProjectTheme"); ?>
					</div>
                		...
                		<?php
							...
							if(count($r) > 0) {
								...
								foreach($r as $row)
								{
									...
									echo '<td class="'.$cls.'">
											<img src="'.ProjectTheme_get_avatar($user_nv->ID,25, 25). '" alt="avatar-user" class="acc_m1" width="25" height="25" />
											<a href="'.ProjectTheme_get_user_profile_link($user_nv->ID).'">'.$user_nv->user_login.'</a>
										</td>';
									echo '<td class="'.$cls.'"><a href="'.ProjectTheme_get_priv_mess_page_url('read-message', $row->id).'">'.$row->subject.'</a></td>';
									echo '<td class="'.$cls.'">'.date_i18n('d-M-Y H:i:s',$row->datemade).'</td>';
									echo '<td><a href="'.ProjectTheme_get_priv_mess_page_url('read-message', $row->id).'" class="green_btn3">'.__('Reply','ProjectTheme').'</a>  
        ...
        <?php }
			elseif($third_page == 'inbox') {
				...
				<div class="my_box3">
					<div class="box_title">
						<?php _e("Private Messages: Inbox","ProjectTheme"); ?>
					</div>
							...
							if(count($r) > 0) {
								...
								<script>
									function toggle(source) {
										checkboxes = document.getElementsByName('message_ids[]');
								...
								<?php
									...
									echo '<tr>';
										echo '<td><input type="checkbox" name=" " id="select_all_stuff" value="1" onClick="toggle(this)"/> '.__('Select All','ProjectTheme').' </td>';
										echo '<td>'.__('From User','ProjectTheme').'</td>';
										echo '<td>'.__('Subject','ProjectTheme').'</td>';
										echo '<td>'.__('Date','ProjectTheme').'</td>';
										echo '<td>'.__('Options','ProjectTheme').'</td>';
		...
		elseif($third_page == 'sent-items') {
			...
			<div class="my_box3">
				<div class="box_title">
					<?php _e("Private Messages: Sent Items","ProjectTheme"); ?>
				</div>
				...
				<div class="padd10">
					...
					<div class="box_content">
						...
						if(isset($_POST['delete_sel'])):
							...
							{
								__("No messsages selected.","ProjectTheme");
							}
							else
							{
								$attash = '';
		...
		elseif($third_page == 'read-message') {
			...
			<div class="clear10"></div>
				...
				<div class="my_box3">
					<div class="box_title">
						<?php _e("Subject: ","ProjectTheme"); echo " ".htmlentities($row->subject) ?>
					</div>
				</div>
						...
						if($uids != false and $error_mm != "1"):
							...
							if(count($rr) == 0) {
								...
								if($ProjectTheme_moderate_private_messages == false) 
									ProjectTheme_send_email_on_priv_mess_received($myuid, $uids,$message, $attach_id);
							}
						...
						<div class="my_box3">
							<div class="box_content">
								<form method="post" enctype="multipart/form-data" action="<?php echo  (isset($_SERVER['HTTPS']) ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]"; ?>">
									...
									<input name="send_to" type="hidden" value="<?php echo $send_to; ?>" />
		...
		elseif($third_page == 'send') { ?>
			...
			<div class="my_box3">
					...
					<td><input class = "inputMessages" size="20" name="to_as" type="text" value="" /> <?php if($rtt): _e('or','ProjectTheme'); echo " ".$rtt; endif; ?></td>
					</tr>
					<?php endif; ?>

					<tr>
					<td width="140"><?php _e("Subject", "ProjectTheme"); ?>:</td>
					<td><input class = "inputMessages" size="50" name="subject_a" type="text" value="<?php echo $subject; ?>" /></td>
					</tr>
				...
				tinyMCE.init({
					...
					<tr>
					<td valign="top"><?php _e("Message", "ProjectTheme"); ?>:</td>
					<td><textarea class = "inputMessages" name="message_a" class="tinymce-enabled" rows="6" cols="50"></textarea></td>
					</tr>


					<tr>
					<td valign="top"><?php _e("Attach File", "ProjectTheme"); ?>:</td>
					<td><input class = "inputMessages" type="file" name="file_instant" class="" /> <?php _e('Only PDF, ZIP, Office files and Images.','ProjectTheme'); ?></td>
					</tr>
