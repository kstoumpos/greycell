...
if(isset($_POST['project_submit1'] )) {
    ...
    $ending 				= trim($_POST['ending']);
    $project_ending 		= trim($_POST['project_ending']);
    $project_category_sub   = $_POST['subcat'];
    ...
    if(empty($ending))  { 
        $projectOK = 0; 
        $MYerror['ending'] 	= __('You cannot leave the bidding period blank!','ProjectTheme');
        $class_errors['ending']		= 'error_class_post_new';  
    }

    if(empty($project_category_sub))  { 
        $projectOK = 0; 
        $MYerror['project_category_sub'] 	= __('You cannot leave the subcategory blank!','ProjectTheme');
        $class_errors['project_category_sub']		= 'error_class_post_new';  
    }
    ...
    $project_skill_cat_multi = $_POST['project_skill_cat_multi'];
    ...
    if(get_option('ProjectTheme_enable_multi_cats') == "yes") {
        ...
        if(!empty($_POST['subcat'])) {
            $term = get_term( $project_category_sub, 'project_cat' );	
    ...
    $ending = strtotime($ending, current_time('timestamp',0));
    $project_ending = strtotime($project_ending, current_time('timestamp',0));
    
    update_post_meta($pid, "made_me_date", 		current_time('timestamp',0));
    
    update_post_meta($pid, "closed", 		"0");
    update_post_meta($pid, "closed_date", 	"0");
    update_post_meta($pid, "ending", 		$ending); // ending date for the bidding period
    update_post_meta($pid, "project_ending", $project_ending); //ending date for project
