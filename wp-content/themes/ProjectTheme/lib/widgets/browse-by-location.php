...
class ProjectTheme_browse_by_location extends WP_Widget {
		     ...
		     foreach ( $terms as $term ) {
			   ...
			   $terms2 = '';
				...
				$mese = '';
					...
					$mese .= '<ul>';
					$mese .= "<img src=\"".get_template_directory_uri()."/images/location.png\" width=\"20\" height=\"20\" /> 
	...
	function form($instance) { ?>
        ...
        <p>
         ...
         <p>
			<label for="<?php echo $this->get_field_id('nr_rows'); ?>"><?php _e('Only show locations below','ProjectTheme'); ?>:</label>
			<?php echo '<input type="checkbox" name="'.$this->get_field_name('only_these').'"  value="1" '.(
	 $instance['only_these'] == "1" ? ' checked="checked" ' : "" ).' /> '; ?>
		</p>
        
        <p>
			<label for="<?php echo $this->get_field_id('nr_rows'); ?>"><?php _e('Locations to show','ProjectTheme'); ?>:</label>
