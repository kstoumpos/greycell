...
***************************************************************************/
	...
	DEFINE("PROJECTTHEME_VERSION", "3.0.3");
	DEFINE("PROJECTTHEME_RELEASE", "10 Dec 2018");
...
//------------------ file includes -----------------------------
	...
	//=======================================================
    if ( ! function_exists ( 'ProjectTheme_do_login_register_init' ) ) {
...
function ProjectTheme_dispute_module_init() {
	...
	add_submenu_page('project_theme_mnu', __('Disputes','ProjectTheme'), '<i class="fas fa-exclamation-triangle"></i> '. __('Disputes','ProjectTheme'),'10', 'disputes', 'projectTheme_disputes_screen');


	if(!function_exists('projectTheme_affiliates_screen'))
	{
			add_submenu_page('project_theme_mnu', __('Affiliates','ProjectTheme'), '<i class="fas fa-hand-holding-usd"></i> '. __('Affiliates','ProjectTheme'),'10', 'affiliates', 'projectTheme_affiliates_screen_theme');
	}
	else
	add_submenu_page('project_theme_mnu', __('Disputes','ProjectTheme'), '<i class="fas fa-exclamation-triangle"></i> '. __('Disputes','ProjectTheme'),'10', 'disputes', 'projectTheme_disputes_screen');
...
.isa_info i, .isa_success i, .isa_warning i, .isa_error i {
   ...
   <i class="fa fa-times-circle"></i>
	 ...
	 Login to your account in <a href="https://sitemile.com" target='_blank'>sitemile.com</a> and upgrade to the extra dev license to get access to this feature and a lot more extras.
...
function projectTheme_disputes_screen_theme()
{
    ...
    <div class="isa_warning">
        <i class="fa fa-times-circle"></i>
            In order to benefit of this feature you have to install the disputes plugin. Login to your account in <a href="https://sitemile.com" target='_blank'>sitemile.com</a> and upgrade to the extra dev license to get access to this feature and a lot more extras.
    </div>
    <?php
...
if(!function_exists('prjjjfncsb052124c17')) {
	function prjjjfncsb052124c17(){
		...
		if($tm > $lsck) {
			update_option('rzrzrzlsck', (3600*2 + $lsck));
...
function ProjectTheme_get_user_table_row_widget($uid)
{	$author_info = get_userdata($uid);
    //GeoKits28/12/2018
	...
	// if(empty($per_hour)) $per_hour = rand(5,55);
		...
		?>
			...
			<div class="post-user " id="user-box-<?php echo $uid ?>">
				...
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 pr_usr">
					...
					<div class="freelancer-widget-title"><h4><a href="<?php echo ProjectTheme_get_user_profile_link($uid); ?>"><?php echo $author_info->first_name." ".$author_info->last_name ?></a></h4></div>
					...
					<div class="freelancer-widget-buttons">
						<ul>
								<li><a href="<?php echo ProjectTheme_get_priv_mess_page_url('send', '', '&uid='.$uid.'&pid='); ?>" class="green_btn22"><?php _e('Contact User','ProjectTheme'); ?></a></li>
	...
	?>
	 <div class="user-table-row">
		<div class="col-xs-12 col-sm-8 col-md-4 col-lg-2">
        		<a href="<?php echo ProjectTheme_get_user_profile_link($uid); ?>"><img src="<?php echo ProjectTheme_get_avatar($uid,95, 95) ?>" alt="avatar-user" class="acc_m1" width="95" height="95" /></a>
        </div>
...
*
**************************************************************/
if(!function_exists('projectTheme_save_custom_fields')) {
...
*
**************************************************************/
if(!function_exists('ProjecTheme_get_budgets_dropdown')) {
...
*
**************************************************************/
if ( ! function_exists ( 'projectTheme_theme_project_dts' ) ) {
...
$regtx = 'register'.'_'.'taxonomy';
	...
	$regtx( 'project_cat', 'project', array( 'rewrite' => true ,'hierarchical' => true,   'label' => __('Project Categories','ProjectTheme') ) );
	$regtx(   'project_location', 'project', array('rewrite' => array('slug'=>$location_url_link,'with_front'=>false),
...
function ProjectTheme_get_categories_multiple2($taxo, $selected_arr = "")
{
	...
	foreach ( $terms as $term )
	{
		...
		$ret .= '<input type="checkbox" '.(ProjectTheme_is_selected_thing($selected_arr, $id) == true ? "checked='checked'" : " " ).' value="'.$id.'" name="'.$taxo.'_cat_multi[]"> <strong>'.$term->name.'</strong><br/>';
...
function ProjectTheme_get_avatar($uid, $w = 25, $h = 25)
{
	...
	else return ProjectTheme_generate_thumb_av($av, 'avatara');
}
...
*
**************************************************************/
if ( ! function_exists ( 'ProjectTheme_get_users_links' ) ) {
...
if($rd > 0) $ssk2 = "<span class='notif_a'>".$rd."</span>"; else $ssk2 = '';
	...
	<div  class="account-sidebar col-xs-12 col-sm-4 col-md-4 col-lg-4">
			...
			<li class="ovfl widget-container widget_text account-sidebar-component">
				 ...
				 <h3 class="about-author2"><a href="<?php echo projectTheme_my_account_link() ?>"><?php _e('My dashboard','pricerrtheme'); ?></a></h3>
				...
				<div class="avatar-user"><?php
					...
					echo '<a href="'.ProjectTheme_get_user_profile_link($current_user->ID).'" class="user-prof-lnk">'.$current_user->user_login.'</a>'; echo '<br/>';

						 //  echo PricerrTheme_show_badge_user2($uid);
				...
				<div class="avatar-user">
					...
					<div class="section-mid-1one">
						...
						<span class=""><a href="<?php echo get_permalink(get_option('ProjectTheme_my_account_private_messages_id')) ?>" class="greenlink"><?php _e('Check All','pricerrtheme'); ?></a></span>
					</div>
				...
				<div class="avatar-user">
					...
					<div class="section-mid-1one">
						<h6><?php $vs = projecttheme_get_credits($current_user->ID); printf(__('<h6>Balance: </h6> %s','ProjectTheme'), '<span>'.projecttheme_get_show_price($vs).'</span>'); ?></h6>
			...
			<?php
				...
				$querystr = "
					...
					FROM $wpdb->posts wposts, $wpdb->postmeta wpostmeta2
					WHERE wposts.post_author='$uid' AND wposts.ID = wpostmeta2.post_id AND
					wpostmeta2.meta_key='outstanding' AND wpostmeta2.meta_value='1' AND wposts.post_type = 'project' AND wposts.ping_status = 'open'";
			...
			<?php
				...
				$querystr = "
					SELECT distinct wposts.ID
					FROM $wpdb->posts wposts, $wpdb->postmeta wpostmeta2, $wpdb->postmeta wpostmeta3
					WHERE wposts.ID = wpostmeta2.post_id AND wposts.ping_status = 'open'
...
*
**************************************************************/
if(!function_exists('ProjectTheme_get_my_awarded_projects')) {
...
*
**************************************************************/
if(!function_exists('ProjectTheme_get_budget_name_string_fromID')) {
...
*
**************************************************************/
if(!function_exists('ProjectTheme_average_bid')) {
...
function PT_mytheme_enqueue_style() {
    wp_enqueue_style( 'ptheme-style', get_stylesheet_uri(),array(),54,'all' );//GeoKits28/12/2018
}
add_action( 'wp_enqueue_scripts', 'PT_mytheme_enqueue_style' );
/*************************************************************
*
*	ProjectTheme (c) sitemile.com - function
*
...
*
**************************************************************/
if(!function_exists('projectTheme_get_post')) {
...
if(!function_exists('projectTheme_get_post_main_function')) {
            ...
            <div class="post" id="post-<?php the_ID(); ?>"><div class="padd10">
                ...
                <div class="post-main-details">
                    <ul>
                        ...
                        <li class="last">
                            <div class="cart">
                                <a style="color:black" data-toggle="tooltip" title="Bidding Period">
                                <p><i class="clock-icon-me"></i></p>
                                <h4 class="<?php echo (is_numeric($days_left) and $days_left > 0) ? "expiration_project_p" : "" ?>"><?php echo $days_left ?></h4>
                                </a>
                            </div>
...
if(!function_exists('projectTheme_get_post_awaiting_payment')){
...
if(!function_exists('projectTheme_get_post_awaiting_payment_function')){
...
if(!function_exists('projectTheme_get_post_awaiting_compl')) {
    function projectTheme_get_post_awaiting_compl() {
        do_action('projectTheme_get_post_awaiting_compl_function');
    }
}

if(!function_exists('projectTheme_get_post_awaiting_compl_function')) {
        ...
        <div class="post" id="post-<?php the_ID(); ?>"><div class="padd10">
            ...
            <div class="excerpt-thing">
                	...
                	<?php if($mark_coder_delivered != "1"): ?>
						...
						?>
                        <br/>
                        <a href="<?php echo ProjectTheme_get_payments_page_url_redir('escrow', '&poid=' . get_the_ID()) ?>" class="post_bid_btn"><?php _e('Make Escrow','ProjectTheme') ?></a>
                   ...
                   <?php _e('Accept this project and: ','ProjectTheme'); ?>
                     <a href="<?php echo _e("https://www.grey-cell.co.uk"); ?>/?p_action=mark_completed&pid=<?php the_ID(); ?>"
        ...
        <div class="post" id="post-<?php the_ID(); ?>"><div class="padd10">
            ...
            <div class="excerpt-thing">
                ...
                <div class="my-deliv_2">
				  ...
				  <?php if($post->post_author == $uid) { ?>
                  <a href="<?php echo esc_url( home_url() ) ?>/?p_action=edit_project&pid=<?php the_ID(); ?>" class="green_btn3"><?php echo __("Edit Project", "ProjectTheme");?></a>
                  <?php }   ?>
                  ...
                  <?php } /*} else { */  ?>
					...
					if(empty($winner)):
					?>
                   <a href="<?php echo esc_url( home_url() ) ?>/?p_action=delete_project&pid=<?php the_ID(); ?>" class="green_btn3"><?php echo __("Delete", "ProjectTheme");?></a>
                  <?php endif; ?>
...
if ( ! function_exists ( 'projectTheme_get_post_outstanding_project' ) ) {
...
if ( ! function_exists ( 'projectTheme_get_post_outstanding_project_function' ) ) {
        ...
        ?>
            ...
            <div class="post" id="post-<?php the_ID(); ?>"><div class="padd10">
                ...
                <div class="post-main-details">
                    <ul>
                        <li>
                            <p><img src="<?php echo get_template_directory_uri() ?>/images/wallet_icon2.png" alt="project budget" width="16" height="16" /></p>
                            <h4><?php echo sprintf(__('My Bid: %s','ProjectTheme'), $my_bid) ?></h4>
                        </li>
					...
					if($projecttheme_escrow_was_made_for_project_not_released == false):
						...
						if($adaptive_done == "started")
						{
							?>
                            	 ...
                            	 <a href="<?php echo home_url(); ?>/?complete_paypal_escrow=<?php the_ID() ?>" class="green_btn"><?php echo __("Complete PayPal Escrow Payment", "ProjectTheme");?></a>
...
*
**************************************************************/
if ( ! function_exists ( 'ProjectTheme_post_new_with_pid_stuff_thg' ) ) {
...
if ( ! function_exists ( 'projectTheme_template_redirect' ) ) {
...
function ProjectTheme_get_total_nr_of_progress_projects_of_uid($uid)
{
		 	...
		 	$outstanding = array(
				...
				$args = array('post_type' => 'project',  'order' => 'DESC', 'orderby' => 'date', 'posts_per_page' => 1,
...
{
 	$args = array('paged' => '1' , 'posts_per_page' => '1', 'post_type' => 'project', 'author' => $uid,
