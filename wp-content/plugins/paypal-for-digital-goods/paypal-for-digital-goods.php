...
function wp_ppdg_process_payment() {
    ...
    $res = array();
    $res[ 'title' ] = 'Η συναλλαγή ολοκληρώθηκε';

    $thank_you_msg = '<div class="wp_ppdg_thank_you_message"><p>Ευχαριστούμε για την αγορά σας.</p><br/></div>';
    $thank_you_msg = apply_filters('wp_ppdg_thank_you_message', $thank_you_msg);
    $res[ 'msg' ] = $thank_you_msg;

    echo json_encode( $res );

    // paypal payment comfirmed, add extra bids to user
    $user_id = get_current_user_id();
    // get id from url
    //$id = $_GET['id'];
    session_start();
    $id = $_SESSION['planId'];
    do_action( 'after_paypal', $id, $user_id);
